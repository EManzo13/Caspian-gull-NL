...
## A) Data preparation
First, we will prepare the data streams for the model. To do so, we will import accelerometer data (containing location information) and compile it in one file per device number, keep only the location when the bird is out of the colony, separate in distinct data sets according to resolution (i.e. the time difference between each data point, which were either 1 minute or 5 minutes), and finally remove outliers.

```{r Prep data}
#Used for when there are several files for the same individual in a folder
...
# data <- import_accelerometer_data("6402")

# Here, only trial file to upload:
Trial <- GET("https://api.github.com/repos/eldarrak/RDM-capture-reencounter-with-misidentification/contents/results")
...
datasets_HMM <- function(data){
  ...
  gullout <- gullout %>%
    mutate(time_diff = counttime - lag(counttime, default = first(counttime))) # recalculate new time diff with selected observations
  gullout <- gullout %>%
  ...
  if(same_device == F){ 
    data <- data %>%
      mutate(device_real = device_info_serial,
             device_info_serial = device_info_serial[1])} #transform be the same device nb for all data or following df_to_list creates 1 dataframe per animal
  
  d.list<- df_to_list(data, ind = "device_info_serial") 
  list1<- map(d.list, ~prepData(., type = "UTM", coordNames = c("x","y")))
  
  stepPar <- as.vector(best_model$mle$step[c(1,3,5,7,2,4,6,8)]) #saves the parameters defined in the best model in the right order to be applied
...
gullsubs_1min$HMM_state <- fit_hmm(gullsubs_1min,best_model1) #adds HMM_states column
gullsubs_5min$HMM_state <- fit_hmm(gullsubs_5min,best_model5)

# Re-assemble the two datasets in one, and link tracks back together 
...
# Specific track for a certain device and date
HMM_states %>% filter(device_info_serial == 6401 & date == "2023-05-16") %>%
  pull(track) %>%
...
feeding_sequences <- function(data = HMM_states,last_id_data = 0){ #last_id_data argument for digitisation of tracks following the ones previously done
  # Keep the ARS sequences and data points around it, removes full Transit sequences 
  data$keep <- FALSE
  n <- nrow(data)
  ...
  # Saves the final table as a .csv table in the working directory
...
The last file produced is used as in input in the behavioural model to obtain the behavioural sequence of each feeding event. The output is one file per event, with each line being a location point and the associated behaviour. The files for each individual is stored in the same folder. 

# 3- Classification of the events
## A) Import and compile
First, we will import and merge all the observations to have one file per individual. It is noteworthy that as the output of this behaviooural model, the files are named with the device number, starting and ending date & time (for example: '6387_2023-05-01 07/32/10_2023-05-01 07/46/34.csv'), which is used here to fill in the columns "device number", "start date & time" and "end date & time". It is then used to match with the events detected by the HMM to obtain the ID of the event that was previously attributed (in the table obtained with the "feeding_sequences()" function).

```{r Merge files}
import_and_merge <- function(folder_name,feeding_sequences_table){ 
  setwd(paste0("path",folder_name))
  ...
  for (i in 1:length(names)){
    behav_event <- read.csv(names[i])
    behav_event$time <- as.POSIXct(behav_event$time)
    
    file_info <- strsplit(names[i], "_|\\.csv")[[1]]
    device <- file_info[1]
...
Trial_behaviour_combined <- import_and_merge("Trial",events_seq_trial)

## Check all events detected by the HMM are imported: number of events per individual
Trial_behaviour_combined %>%
  group_by(device) %>%
  summarise(n_events_seq = n_distinct(event_id)) %>% 
  bind_cols(events_seq %>% group_by(device_info_serial) %>% 
              summarise(n_events_import = n()))
```
...
```{r Filter and compute proportions}
filter_behav <- function(data){
  # Mutate the prediction column based on confidence and altitude
...
behaviour_proportions <- function(data, feeding_sequences_table) {
  ...
  reshaped_proportions <- reshaped_proportions[-which(reshaped_proportions$Flap + reshaped_proportions$ExFlap + reshaped_proportions$Soar + x = reshaped_proportions$Manouvre == 100),]
  reshaped_proportions <- reshaped_proportions %>% arrange(start_time)
  reshaped_proportions <- reshaped_proportions %>% mutate(event_id = row_number())

  return(reshaped_proportions)
}
Trial_summary <- behaviour_proportions(Trial_filtered, feeding_sequences_table)

```

## C) Classification
As explained in the thesis, the classification is done partially based on location (for events in a waste management plant, in or on the coast of the Ijsselmeer and in smaller inland lakes) or based on behaviour proportions (for events in agricultural fields or canals).
The lakes polygons are obtains from the HydroLakes database, the waste management plants polygons are created in the following code, and the LDA model based on behaviour to differenciate foraging on agricultural fields from canals is trained on a trial dataset annotated by hand (see "Behaviour_analysis" code file).
...
```{r classification}
# Load required datasets, model and function
load_classification_objects <- function(){
  setwd("/Users/emiliemanzo/Desktop/Internship/Models")
  lakes_buffer <<- readRDS("lakes_buffer.rds")
  lakes_buffer <<- st_make_valid(lakes_buffer)
  NL_lakes <<- readRDS("lakesNL.rds")
  ijsselmeer <<- NL_lakes %>%
    filter(Lake_name == 'Ijsselmeer')
  st_crs(ijsselmeer) <<- st_crs(lakes_buffer)
  ijsselmeer <<- st_make_valid(ijsselmeer)
  ijsselmeer_coast <<- readRDS("ijsselmeer_coast.rds")
