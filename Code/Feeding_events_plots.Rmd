...
Plotting function allowing for visualisation of the animal's route and behaviour while foraging (used during data observation).   
To be applied to a dataset of all GPS points within a feeding event, for which the temperature and the altitude are measured as well. An API google key must be obtained to have a background map in the track plot.
